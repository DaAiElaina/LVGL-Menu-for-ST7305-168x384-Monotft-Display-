...
#define ST7305_COLOR_WHITE false
#define ST7305_COLOR_BLACK true
...
public:
    ...
    void display();
    // 在 ST7305_2p9_BW_DisplayDriver.h 的 public 部分添加
    // ST7305_2p9_BW_DisplayDriver.h
    void writeBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h);
    void Initial_ST7305();
    void Low_Power_Mode();
    void High_Power_Mode();
    void display_on(bool enabled);
    void display_Inversion(bool enabled);
    void drawBitmapFast(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color);
private:
    const int DC_PIN;
    const int RES_PIN;
    const int CS_PIN;
