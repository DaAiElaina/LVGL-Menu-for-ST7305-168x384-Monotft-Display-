...
// 在 ST7305_2p9_BW_DisplayDriver.cpp 中添加
void ST7305_2p9_BW_DisplayDriver::writeBitmap(int16_t x, int16_t y, 
                                               const uint8_t *data, 
                                               int16_t w, int16_t h) {
    // 直接操作 display_buffer，绕过 writePoint 的逐像素开销
    for (int16_t yy = 0; yy < h; yy++) {
        for (int16_t xx = 0; xx < w; xx++) {
            uint8_t byte = data[yy * ((w + 7) / 8) + (xx / 8)];
            bool pixel = (byte & (0x80 >> (xx % 8))) != 0;
            
            if (pixel) {
                // 这里复制 writePoint 的计算逻辑，但直接写 buffer
                uint real_x = (x + xx) / 4;
                uint real_y = (y + yy) / 2;
                uint write_byte_index = real_y * LCD_DATA_WIDTH + real_x;
                uint one_two = ((y + yy) % 2 == 0) ? 0 : 1;
                uint line_bit_4 = (x + xx) % 4;
                uint8_t write_bit = 7 - (line_bit_4 * 2 + one_two);
...
void ST7305_2p9_BW_DisplayDriver::drawBitmapFast(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color) {
    // 直接操作 display_buffer，每个像素只做位运算，不调用函数
    for (int16_t yy = 0; yy < h; yy++) {
        for (int16_t xx = 0; xx < w; xx++) {
            uint8_t byte = bitmap[yy * ((w + 7) / 8) + (xx / 8)];
            bool pixel = (byte & (0x80 >> (xx % 8))) != 0;
            
            if (pixel) {
                // 直接复制 writePoint 的计算逻辑（内联，无函数调用开销）
                uint real_x = (x + xx) / 4;
                uint real_y = (y + yy) / 2;
                uint write_byte_index = real_y * LCD_DATA_WIDTH + real_x;
                uint one_two = ((y + yy) % 2 == 0) ? 0 : 1;
